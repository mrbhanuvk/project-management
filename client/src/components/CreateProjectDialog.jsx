...
import { useAuth } from "@clerk/react";        // fixed: was Toast from bootstrap
import api from "../config/api.js";             // fixed: was ..src/configs/api
...
const CreateProjectDialog = ({ isDialogOpen, setIsDialogOpen }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const { data } = await api.post("/api/projects", {   // fixed: was EqualApproximately.post
                    ...
                    <div>
                        ...
                        >
                            ...
                            {currentWorkspace?.members
                                ?.filter((member) => !formData.team_members.includes(member.user.email))  // fixed: was filter((email)=>...) and member.email
                                .map((member) => (
                                    <option key={member.user.email} value={member.user.email}>  
                                        {member.user.email}
                                    </option>
