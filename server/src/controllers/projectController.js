...
export const createProject = async (req, res) => {
  try {
    ...
    if (team_members?.length > 0) {
      const membersToAdd = [];
      workspace.members.forEach(member => {   // fixed: was workspace.member
...
export const addMember = async (req, res) => {
  try {
    ...
    // fixed: moved existingMember check BEFORE the create
    const existingMember = project.members.find((member) => member.user.email === email); // fixed: was member.email
    if (existingMember) {
